...
const withPackageRegistration = (config) =>
  withMainApplication(config, (cfg) => {
    ...
    if (/PackageList\(this\)\.packages\.apply\s*\{/.test(src)) {
      ...
      // Expo 52 / RN 0.76 shape: `val packages = PackageList(this).packages` ... `return packages`
      src = src.replace(/return\s+packages\b/, `packages.add(com.receiverandroid.LogosMessagingPackage())\n            return packages`);
    } else {
